...
class Monitor:
    def __init__(self, checkpoint, plotly=True, hosting_folder=None):
        ...
        logio.create_log_file(self.train_path)
        logio.create_log_file(self.val_path)
    ...
    def log_train(self, epoch, errors):
        logio.log_errors(epoch, errors, self.train_path)

    def log_val(self, epoch, errors):
        logio.log_errors(epoch, errors, self.val_path)
...
class Metrics:
    ...
    def plot_metrics(self):
        ...
        if self.hosting_folder is not None:
            hosted_plotly_path = os.path.join(
                self.hosting_folder, "plotly.html"
            )
